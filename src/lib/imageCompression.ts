...
export const compressImage = async (
  ...
  const { maxSizeInMB = 1, maxWidthOrHeight = 1920, quality = 0.8 } = options;
  ...
  return new Promise((resolve, reject) => {
    const attemptCompression = (
      ...
      console.log(
        `🔄 Attempt ${attempt}: Trying quality ${currentQuality.toFixed(2)}`
      );
      ...
      new Compressor(file, {
        ...
        maxWidth:
          attempt === 1
            ? maxWidthOrHeight
            : Math.max(800, maxWidthOrHeight * (1 - attempt * 0.1)),
        maxHeight:
          attempt === 1
            ? maxWidthOrHeight
            : Math.max(600, maxWidthOrHeight * (1 - attempt * 0.1)),
        convertSize: 1000000, // Convert PNG to JPEG if larger than 1MB (more aggressive)
        convertTypes: ['image/png', 'image/webp'],
        mimeType:
          attempt >= 2
            ? 'image/jpeg'
            : file.type === 'image/png' && file.size > 1000000
              ? 'image/jpeg'
              : file.type,
        ...
        success: (compressedFile: File) => {
          ...
          console.log(
            `📊 Compressed to: ${compressedSizeMB.toFixed(2)}MB (${compressedFile.type}, ${compressedFile.name})`
          );

          // If compressed file meets size requirement, return it
          if (compressedFile.size <= targetSizeBytes) {
            console.log(
              `✅ Success! Final size: ${compressedSizeMB.toFixed(2)}MB`
            );
          ...
          if (attempt >= 6 || currentQuality <= 0.1) {
            console.log(
              `❌ Failed after ${attempt} attempts. Final size: ${compressedSizeMB.toFixed(2)}MB`
            );
            reject(
              new Error(
                `Unable to compress image below ${maxSizeInMB}MB. Please use a smaller image or lower resolution.`
              )
            );
            return;
          }
          ...
          console.log(
            `🔄 Still ${compressedSizeMB.toFixed(2)}MB, trying again with quality ${nextQuality.toFixed(2)}`
          );
